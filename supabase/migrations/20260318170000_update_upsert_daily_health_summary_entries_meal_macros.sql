-- Update meal aggregation so daily_health_summary captures protein/carbs/fat from entries
-- (after LLM nutrition extraction writes these columns).
...
begin
  ----------------------------------------------------------------------
  -- 1) MEAL SIGNALS (entries.note_type = 'meal')
  ----------------------------------------------------------------------
  with meal_entries as (
    select
      e.*,
      -- user-local timestamp using same timezone logic as whoop_day_for_ts
  ...
  select
    -- late_meal: NULL if there are no meal rows
    case
    ...
    -- calories_kcal: NULL if there are no meal rows
    case
      when count(*) = 0 then null
      else sum(coalesce(m.calories_kcal, 0))::double precision
    end as calories_kcal,
    -- macronutrients: NULL if there are no meal rows
    case
      when count(*) = 0 then null
      else sum(coalesce(m.protein_g, 0))::double precision
    end as protein_g,
    case
      when count(*) = 0 then null
      else sum(coalesce(m.carbs_g, 0))::double precision
    end as carbs_g,
    case
      when count(*) = 0 then null
      else sum(coalesce(m.fat_g, 0))::double precision
    end as fat_g
  into
    v_late_meal,
    v_calories_kcal,
  ...
  ----------------------------------------------------------------------
  -- 2) SYMPTOM SIGNALS (entries.note_type = 'symptom')
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 3) STATE SIGNALS (entries.note_type = 'state')
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 4) UPSERT INTO daily_health_summary
  ----------------------------------------------------------------------
